...
// social media
if ( ! function_exists( 'lifeleck_social' ) ) {
	function lifeleck_social( $args = array()  ){
		...
		$url = lifeleck_opt('lifeleck_social_url');
...
function lifeleck_contact_form7_shortcode(){

    // contact form list
    $getforms['cs'] = __( 'Custom Shortcode', 'lifeleck' );
    // Instruction
    $Instruction = ''; 
    ...
    if( defined('WPCF7_VERSION') ){
        ...
        if( $loop->have_posts() ){
            ...
            $Instruction = __( 'Contact form not found.', 'lifeleck' );
        }
    }else{
        $url = admin_url( 'plugins.php' );
        
        $Instruction = sprintf( __( 'If you want to use contact form 7, Please install and active contact form 7 plugin. %s Click here to install %s  ' , 'lifeleck' ), '<a target="_blank" href="'.esc_url( $url ).'">', '</a>'  );
    }
...
// Popular post count
function lifeleck_set_post_views($postID) {
    $count_key = 'lifeleck_post_views_count';
...
// blog post categoty 
function lifeleck_get_post_cat(){
    $cats = get_categories();

    $categories = array( 'na' => esc_html__( 'Select post category', 'lifeleck' ) );
